...
def main(new_poi_id, old_poi_id, cookie, result):
    ...
    for i in collection_old.find():
        ...
        if document_new and 'errMsg' in document_new and document_new['errMsg'] == '':
            ...
            if document_new['originPrice'] - price <= 0:
                result.add(f'折扣价格大于原价 {name}, {spec}')
                continue
            if document_new['originPrice'] <= 0 or price <= 0:
                result.add(f'价格不能为负数 {name}, {spec}')
                continue

            actlist.append(
                set_foods(document_new['skuId'], document_new['spuId'], document_new['originPrice'], price)
            )
        else:
