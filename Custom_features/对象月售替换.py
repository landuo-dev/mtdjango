...
class ObjectReplace():
    def __init__(self, poi_id, cookie):
        ...
        self.collection_act = db_act[poi_id]
        self.collection_pro = db_pro[poi_id]
        self.poi_id = poi_id
        self.cookie = cookie
    ...
    def replace_object(self):
        flog = 1
        # 获取对象
        object = self.collection_pro.find_one({"name": self.collection_pro['name']})
        if object is None:
            print(f"对象 {self.collection_pro['name']} 不存在")
            flog = 0
        return flog
    def get_time(self):
        # 获取当前日期和时间的datetime对象
    ...
    def set_post_data(self, wmSkuId, wmSpuId, originPrice, actPrice):
        data = {
    ...
    def get_actdata(self, poiId, tagId, startTime, endTime, tagname, cookie):
        quer_data = {
            'source': 'pc',
            'actType': '17',
            'poiId': poiId,
    ...
    def rep_pro(self, poi_id, name_x, name_y, act_name_x, act_name_y, cookie, result):
        # result = set()
        client = MongoClient('mongodb://localhost:27017/')
        db_pro = client['test']
        db_act = client['actproduct']
...
def run(poi_id, result, cookie):
    replace = ObjectReplace(poi_id, cookie)
    replace.get_time()
    if not replace.replace_object():
        result.add(f'{poi_id} 替换失败')
